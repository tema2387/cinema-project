    ...
    <div class="demo">
        <div class="container">
            ...
            <ul>
                ...
                <li><a href="#accordeon">Аккордеон</a></li>
            </ul>
            ...
            <p>
                <code>$global</code> - доступ к глобальным методам, (можно было бы подключать миксины, но я не хочу),
                все методы описываются в файле <code>~/plugins/global.js</code> путем добавления новых свойств в объект
                <code>global</code>
            </p>
            ...
            <p>Добавлен файл <code>./static/robots.txt</code> с дефолтным содержимым</p>
            ...
            <div class="example">
                ...
                <pre>
                    ...
                    .foo{
                        display: block; // на больших (от @largeStart) - блок
                        @media @md{
                            display: inline; // на средних (от @mediumStart до @medium) инлайн
                            .bar{
                                display: block;
                            }
                        }
                        @media @sm{
                            display: none; // на малых (до @small) скрыто
                        }
                        @media @notsm{
                            background: #ccc // не на малых экранах (от @mediumStart) - серый фон
                        }
                        @media @supersm{
            ...
            <p>
                ...
                <code>~/assets/less/vars.less {@siteWidth}</code>
            </p>
            <p>
            ...
            <ul>
                ...
                <li><code>centered: Boolean (false)</code> - центрует колонку посередине ряда</li>
            ...
            <ul>
                <li>
                    <code>size: String ('xxl'|'xl'|'l'|'n'|'s')</code>
                </li>
                <li>
                ...
                <li>
                    <code>[color]: String ('')</code> - название цвета кнопки, при необходимости дописывается в
            ...
            <div class="example">
                <pre
                    v-text="
                        ...
                        <div class='flex'>
                            ...
                            <UiButton size='l'>большая кнопка</UiButton>
                            <UiButton  size='s'>маленькая кнопка</UiButton>
                            <UiButton hollow>Кнопка с обводкой</UiButton>
                            <UiButton disabled green>заблокированная кнопка</UiButton>
                            <UiButton wide color='dark'>широкая с иконкой <svg-icon name='close' width='24' height='24' /></UiButton>
                        </div>
                        `
                    "
                ></pre>
                <div class="flex-c" style="flex-wrap: wrap">
                    ...
                    <UiButton hollow>Кнопка с обводкой</UiButton>
                    <UiButton disabled green>заблокированная кнопка</UiButton>
                    <UiButton wide color="dark"
                        >тёмная широкая с иконкой <svg-icon name="close" width="24" height="24"
                    /></UiButton>
                </div>
            </div>
                                ...
                                form: {
                                    ...
                                    submit: {                                  // Кнопка отправки формы
                                        ...
                                        callback: async () => {
                                            ...
                                            try {
                                                const res = await this.$axios.post('/some-url')
                                                console.log('res =>', res)
                                            } catch (error) {
            ...
            <div class="example">
                ...
                <pre
                    v-text="
                        `
                        <UiButton color='dark' @click.native='isModalOpen = true'
                    ...
                    "
                ></pre>
                <UiButton color="dark" @click.native="isModalOpen = true">Открыть модальное окно</UiButton>
                <UiModal v-model="isModalOpen" :open="isModalOpen" position="top right">
                    <h3>Простое модальное окно</h3>
                </UiModal>
            </div>
            <h2 id="accordeon">Аккордеон</h2>
            <p>
                <code>UiAccordeon</code> - раскрывающийся блок, отслеживает изменение контента и подстраивает высоту под
                него, атрибуты:
            </p>
            <ul>
                <li>
                    <code>state: Boolean (false)</code>
                    - состояние аккордеона, открыто/закрыто
                </li>
                <li>
                    <code>initState: String ('')</code>
                    - начальное состояние, если нужно раскрыть сразу при появлении
                </li>
                <li>
                    <code>heightMod: Number (0)</code>
                    - модификатор высоты, для корректировки автоматически расчитанной высоты
                </li>
            </ul>
            <div class="example">
                <pre
                ...
                <pre
                    v-text="
                        `
                        <UiAccordeon :state='accState' @toggle='accState = $event'>
                            <template #label>
                                some label
                            </template>
                            <template #content>
                ...
                ></pre>
                <UiAccordeon :state="accState" @toggle="accState = $event">
                    <template #label> <UiButton color="dark">Открыть</UiButton> </template>
                    <template #content>
                        Lorem ipsum dolor sit amet consectetur adipisicing elit. Dignissimos, aliquid nam? Suscipit
                        fugiat quidem eius vitae vero soluta, ex eveniet amet. Corrupti laudantium labore provident ea
                        eligendi consequatur numquam nobis.
                    </template>
                </UiAccordeon>
            </div>
        </div>
